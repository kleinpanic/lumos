...
static char *load_saved_interface(void) {
    ...
    char buf[256];
    if (!fgets(buf, sizeof(buf), f)) { fclose(f); free(cfg); return NULL; }
    fclose(f); free(cfg);
    buf[strcspn(buf, "\n")] = '\0';
    return strdup(buf);
}
...
static void print_usage(const char *prog) {
    fprintf(stderr,
      ...
      "  -x, --external <connector>   DDC/CI external monitor\n"
...
int main(int argc, char *argv[]) {
    ...
    if (opt_ext) {
        ...
        if (opt_r) {
            ...
            printf("External '%s': raw=%d, max=%d, percent=%d%%\n",
                   opt_ext, cur, mx, (cur * 100) / mx);
            free(i2c);
            return EXIT_SUCCESS;
        }
        ...
        if (opt_s != -1) {
            ...
            if ((opt_s < 0) || (!opt_R && opt_s > 100))
                fprintf(stderr, "Warning: clamped to %d%%\n", val);
            int r = set_ddc_brightness(i2c, val);
            if (!r)
                printf("External '%s' set to %d%% (raw=%d)\n",
                       opt_ext, val, val);
            free(i2c);
            return r ? EXIT_FAILURE : EXIT_SUCCESS;
        }
    ...
    if (opt_r) {
        ...
        printf("Internal backlight: raw=%d, max=%d, percent=%d%%\n",
               cur, mx, (cur * 100) / mx);
        return EXIT_SUCCESS;
    }
    if (opt_m) {
